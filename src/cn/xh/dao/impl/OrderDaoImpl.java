...
public class OrderDaoImpl implements OrderDao {
	...
	public void save(Order o) {
		try {
			...
			for (Orderitem item : items) {
				...
				addXiaonumber(item.getBook().getBook_id(),item.getQuantity());
	...
	//生成订单同时增加相应书籍的销量并减少库存
	private void addXiaonumber(String book_id,int nums) {
		try {
			Connection connection = JDBCUtil.getConnection();
			PreparedStatement prepareStatement = connection.prepareStatement(
					"update bookdb set book_xiaonumber = book_xiaonumber+?,book_kunumber=book_kunumber-? where book_id=?");
			prepareStatement.setLong(1, nums);
			prepareStatement.setLong(2, nums);
	...
	public List<Order> findOrdersByUser(User user) {
		try {
			PreparedStatement prepareStatement = connection.prepareStatement("select * from orders where userId = ? order by ordernum desc");
